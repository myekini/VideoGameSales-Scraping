...
def scrape_vgchartz_data(pages=25):
    """
    Scrape data from VGChartz website.
    ...
    for goty_year in range(2007, 2023):
        for current_genre in genres:
            ...
            url_tail = f'&console=&region=All&developer=&publisher=&genre={current_genre}&boxart=Both&ownership=Both'
...
# Run the function
df_result = scrape_vgchartz_data()
df_result.to_csv("test.csv", sep=",", encoding='utf-8', index=False)
